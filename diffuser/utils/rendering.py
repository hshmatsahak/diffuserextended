...
class MazeRenderer:
    ...
    def dual_renders(self, path1, path2, title=None, **kwargs):
        plt.clf()
        fig = plt.gcf()
        fig.set_size_inches(5, 5)
        ...
        colors = plt.cm.jet(np.linspace(0,1,path_length))
        plt.plot(path1[:,1], path1[:,0], c='blue', zorder=10)
        plt.scatter(path1[:,1], path1[:,0], c=colors, zorder=20)
        plt.plot(path2[:,1], path2[:,0], c='red', zorder=10)
        plt.scatter(path2[:,1], path2[:,0], c=colors, zorder=20)
        plt.axis('off')
        plt.title(title)
        img = plot2img(fig, remove_margins=self._remove_margins)
        return img
    ...
    def composite(self, savepath, paths, ncol=5, **kwargs):
        ...
        '''
        assert len(paths) % ncol == 0, f'Number of paths must be divisible by number of columns but len(paths) is {len(paths)}'
    ...
    def dual_composite(self, savepath, paths1, paths2, ncol=5, **kwargs):
        """
        Renders and saves a composite image of two sets of trajectories per maze.
        
        Args:
            savepath (str): Path to save the rendered image.
            paths1 (list[np.ndarray]): List of first trajectories, each of shape [horizon, 2].
            paths2 (list[np.ndarray]): List of second trajectories, each of shape [horizon, 2].
            ncol (int): Number of columns in the composite image grid.
            kwargs: Additional keyword arguments for rendering.
        """
        assert len(paths1) == len(paths2), "Paths1 and Paths2 must have the same number of trajectories."
        assert len(paths1) % ncol == 0, "Number of paths must be divisible by the number of columns."

        images = []
        for path1, path2 in zip(paths1, paths2):
            img = self.dual_renders(path1, path2, **kwargs)
            images.append(img)
        images = np.stack(images, axis=0)
        ...
        imageio.imsave(savepath, images)
        print(f"Saved dual trajectory samples to: {savepath}")
...
class Maze2dRenderer(MazeRenderer):
    ...
    def dual_renders(self, path1, path2, conditions=None, **kwargs):
        bounds = MAZE_BOUNDS[self.env_name]

        path1, path2 = path1 + .5, path2 + .5
        if len(bounds) == 2:
            _, scale = bounds
            path1 /= scale
            path2 /= scale
        ...
        if conditions is not None:
            conditions /= scale
        return super().dual_renders(path1, path2, conditions, **kwargs)


class Maze2DRendererWithObstacles(Maze2dRenderer):
    def __init__(self, env, observation_dim=None, obstacle_color='red', obstacle_marker='X'):
        """
        Renderer for Maze2D with support for rendering obstacles from conditions.

        Args:
            env (str): Maze2D environment name.
            observation_dim (int, optional): Dimension of observations.
            obstacle_color (str): Color of the obstacle markers.
            obstacle_marker (str): Marker style for obstacles.
        """
        super().__init__(env, observation_dim)
        self.obstacle_color = obstacle_color
        self.obstacle_marker = obstacle_marker
    
    def renders(self, observations, obstacles, **kwargs):
        bounds = MAZE_BOUNDS[self.env_name]
        observations = observations + .5
        if len(bounds) == 2:
            _, scale = bounds
        ...
        # Ensure obstacles are a NumPy array
        if isinstance(obstacles, torch.Tensor):
            obstacles = obstacles.cpu().numpy()  # Convert from CUDA tensor to NumPy
        obstacles[:, 0] = (obstacles[:, 0] + 1) * 0.5 
        obstacles[:, 1] = (obstacles[:, 1] + 1) * 0.5
        ...
        plt.plot(observations[:,1], observations[:,0], c='black', zorder=10)
        plt.scatter(observations[:,1], observations[:,0], c=colors, zorder=20)
        plt.scatter(obstacles[:, 1], obstacles[:, 0], c='red', marker='X', s=150, zorder=30, label='Obstacles')
        plt.axis('off')
        img = plot2img(fig, remove_margins=self._remove_margins)
        return img

    def composite(self, savepath, paths, conditions, ncol=5, **kwargs):
        """
        Save a composite image of paths and conditions.

        Args:
            savepath (str): Path to save the rendered image.
            paths (np.ndarray): Paths (trajectories) to render, shape [n_paths, horizon, 2].
            conditions (np.ndarray): Conditions for each path, shape [n_paths, 1, 2, 2].
            ncol (int): Number of columns in the composite image grid.
        """
        n_paths = paths.shape[0]  # Get number of paths from the first dimension
        assert n_paths % ncol == 0, f"Number of paths must be divisible by ncol, but got {n_paths} paths and ncol={ncol}"

        images = []
        for path, condition in zip(paths, conditions):
            img = self.renders(path, obstacles=condition, **kwargs)
            images.append(img)
        ...
        imageio.imsave(savepath, images)
        print(f"Saved {n_paths} samples to: {savepath}")
